...
test('Testing search results componentWillReceiveProps - Happy 1', async () => {
    const handleClick = jest.fn();
    const handlePagination = jest.fn();
    const sorter = { sortApplied: true, sortParam: "rating-desc" };
    ...
    const wrapper = shallow(<CHSearchContent updateContent={handleClick} updatePage={handlePagination}
        firstName={"Test1"} email={"test1@test.com"} searchString={"test"}
        pageNumber={1} filters={filterparams} sorter={sorter} />);
    const response = {
        "number_of_pages": 43,
        "current_page": 1,
        "courses": [
    ...
    instance.componentWillReceiveProps({ searchString: "test", pageNumber: 1, filters: filterparams, sorter: sorter });
    expect(wrapper.exists()).toBe(true);
});


test('Testing search results componentWillReceiveProps - Happy 2', async () => {
    const handleClick = jest.fn();
    const handlePagination = jest.fn();
    const sorter = { sortApplied: true, sortParam: "rating-desc" };
    const filterparams = { "filtersApplied": false, "startDate": '2017-08-08', endDate: '2018-08-08', minPrice: 0, maxPrice: 10, courseProviders: ['EDX'] }
    const wrapper = shallow(<CHSearchContent updateContent={handleClick} updatePage={handlePagination}
        firstName={"Test1"} email={"test1@test.com"} searchString={"test"}
        pageNumber={1} filters={filterparams} sorter={sorter} />);
    const response = {
...
test('Testing search results componentWillReceiveProps - Happy 3', async () => {
    const handleClick = jest.fn();
    const handlePagination = jest.fn();
    const sorter = { sortApplied: false, sortParam: "rating-desc" };
    const filterparams = { "filtersApplied": true, "startDate": '2017-08-08', endDate: '2018-08-08', minPrice: 0, maxPrice: 10, courseProviders: ['EDX'] }
    const wrapper = shallow(<CHSearchContent updateContent={handleClick} updatePage={handlePagination}
        firstName={"Test1"} email={"test1@test.com"} searchString={"test"}
        pageNumber={1} filters={filterparams} sorter={sorter} />);
...
test('Testing Sort By Price On-click feature', async () => {
    const handleClick = jest.fn();
    const handleSort = jest.fn();
    const wrapper = shallow(<CHSearchContent updateContent={handleClick} updateSort={handleSort} firstName={"Test1"} email={"test1@test.com"} searchString={"test"} pageNumber={1} />);
    const instance = wrapper.instance();
    wrapper.find('#search-results-table-header-sort-item-price').simulate("click");
});

test('Testing Sort By Rating On-click feature', async () => {
    const handleClick = jest.fn();
    const handleSort = jest.fn();
    const wrapper = shallow(<CHSearchContent updateContent={handleClick} updateSort={handleSort} firstName={"Test1"} email={"test1@test.com"} searchString={"test"} pageNumber={1} />);
    const instance = wrapper.instance();
    wrapper.find('#search-results-table-header-sort-item-rating').simulate("click");
});

/* test('Testing Course Name On-click feature', async () => {
    const handleClick = jest.fn();
    const handlePagination = jest.fn();
    const handleSort = jest.fn();
    const sorter = { sortApplied: true, sortParam: "rating-desc" };
    const filterparams = { "filtersApplied": false, "startDate": '2017-08-08', endDate: '2018-08-08', minPrice: 0, maxPrice: 10, courseProviders: ['EDX'] }
    const wrapper = shallow(<CHSearchContent updateContent={handleClick} updateSort={handleSort} updatePage={handlePagination}
        firstName={"Test1"} email={"test1@test.com"} searchString={"test"}
        pageNumber={1} filters={filterparams} sorter={sorter} />);
    const response = {
        "number_of_pages": 1,
    ...
    const instance = wrapper.instance();
    instance.componentWillReceiveProps({ searchString: "test", pageNumber: 1, filters: filterparams, sorter: sorter });
    wrapper.find('#search-results-course-data-name-link-0').simulate("click");
}); */